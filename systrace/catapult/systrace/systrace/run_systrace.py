...
benchmark_cmd = "adb shell am instrument -w -r    -e debug false -e class " \
                "'com.example.android.testing.uiautomator.FrameBench.AutoUITest' " \
...
def start_benchmark(options):
    ...
    old_path = os.getcwd()
    proc = Popen(benchmark_cmd, stdin=PIPE, stdout=PIPE, shell=True)
    ...
    Popen("adb pull /sdcard/Download/timeLine.json " + os.path.dirname(options.output_file), shell=True)
...
def main_impl(arguments):
    ...
    out_file_dir = os.path.split(options.output_file)[0]
