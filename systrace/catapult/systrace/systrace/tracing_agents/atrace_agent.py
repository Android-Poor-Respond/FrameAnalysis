...
class AtraceAgent(tracing_agents.TracingAgent):
    ...
    def __init__(self, device_sdk_version):
        ...
        self._trace_data_dump = ""
        self._trace_data_dump_flag = True
    ...
    def StartAgentTracing(self, config, timeout=None):
        ...
        if os.path.exists(self._config.output_file):
            os.remove(self._config.output_file)
        self._periodic_thread = threading.Thread(target=self._periodic_dump_traces)
        self._periodic_thread.start()
        return True

    def _periodic_dump_traces(self):
        """Periodically clear and dump the ring buffer of atrace.

        Data is dumped to a temp file named output_file.temp
        """
        self._data_file = open(self._config.output_file, 'w+')
        while True:
            # sleep_time = 20 - process_time if process_time <= 20 else 0
            time.sleep(10)
            if not self._trace_data_dump_flag:
                break
            self._config.controller.IssueClockSyncMarker()
            self._trace_data_dump = self._dump_trace_data()
            self._data_file.write(self._trace_data_dump)
            self._config.controller.controller_agent.StopAgentTracing()
            self._controller_data.append(self._config.controller.controller_agent.GetResults())
            self._config.controller.controller_agent.StartAgentTracing()
            self._trace_data_dump = ""


    def _dump_trace_data(self):
        trace_data = self._device_utils.RunShellCommand(
            self._tracer_args + ['--async_dump'], raw_output=True,
            large_output=True, check_return=True,
            timeout=ADB_LARGE_OUTPUT_TIMEOUT)
        # self._device_utils.RunShellCommand(
        #     self._tracer_args + ['--async_start'], check_return=True)
        data_start = re.search(TRACE_START_REGEXP, trace_data)
        if data_start:
            data_start = data_start.end(0)
        else:
            raise IOError('Unable to get atrace data. Did you forget adb root?')
        trace_data = re.sub(ADB_IGNORE_REGEXP, '', trace_data[data_start:])
        trace_data = self._preprocess_trace_data(trace_data)
        # if self._trace_data_dump == "":
        #     trace_data = '\n'.join(trace_data[2:]) + '\n'
        # else:
        #     trace_data = '\n'.join(trace_data[11:]) + '\n'
        return trace_data
    ...
    def _collect_and_preprocess(self):
        ...
        """
        self._trace_data_dump_flag = False
        self._periodic_thread.join()
        self._periodic_thread = None
        self._config.controller.IssueClockSyncMarker()
        trace_data = self._collect_trace_data()
        # # check whether to remove header
        # if self._trace_data_dump != "":
        #   trace_data = '\n'.join(trace_data.split('\n')[12:])
        self._config.controller.controller_agent.StopAgentTracing()
        self._controller_data.append(self._config.controller.controller_agent.GetResults())
        self._config.controller.controller_agent.StartAgentTracing()
        # trace_data = self._trace_data_dump + trace_data
        self._trace_data = self._preprocess_trace_data(trace_data)

        if self._data_file is None:
            self._data_file = open(self._config.output_file, 'w+')
        self._data_file.write(self._trace_data)
        self._data_file.close()
        self._trace_data = self._controller_data
    ...
    def RecordClockSyncMarker(self, sync_id, did_record_sync_marker_callback):
        ...
        self._device_utils.RunShellCommand(
            cmd, raw_output=True, large_output=False, check_return=True,
            timeout=ADB_LARGE_OUTPUT_TIMEOUT, shell=True)
    ...
    def _preprocess_trace_data(self, trace_data):
        ...
        """
        if trace_data:
            if '-z' in self._tracer_args:
                trace_data = strip_and_decompress_trace(trace_data)
            else:
                trace_data = strip_trace(trace_data)
...
class AtraceConfig(tracing_agents.TracingConfig):
    def __init__(self, atrace_categories, trace_buf_size, kfuncs,
                 ...
                 device_serial_number, trace_time, target, output_file):
...
def get_config(options):
    return AtraceConfig(options.atrace_categories,
                        ...
                        options.trace_time, options.target, options.output_file)
