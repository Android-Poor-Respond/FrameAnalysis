...
class TracingControllerAgent(tracing_agents.TracingAgent):
  ...
  def StartAgentTracing(self, config=None, timeout=None):
...
class TracingController(object):
  def __init__(self, agents_with_config, controller_config):
    ...
    self.controller_agent = TracingControllerAgent()
  ...
  def StartTracing(self):
    ...
    if not self.controller_agent.StartAgentTracing(
  ...
  def StopTracing(self):
    ...
    if not self.controller_agent.StopAgentTracing(
        timeout=self._controller_config.timeout):
    ...
    all_results = []
    for agent in self._child_agents + [self.controller_agent]:
      try:
