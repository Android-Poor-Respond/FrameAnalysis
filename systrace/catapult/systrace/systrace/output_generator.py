...
def GenerateHTMLOutput(trace_results, output_file_name):
    ...
    # line endings, then write the prefix.
    file_count = 0
    file_prefix = '/'.join(output_file_name.split("/")[0:-1]) + '/' + output_file_name.split('/')[-1].split('.')[0]
    file_suffix = "html"
    html_file = None

    temp_file_name = output_file_name + '.temp'
    ...
    if os.path.exists(temp_file_name):
        with open(temp_file_name, 'r+') as temp_file:
            while True:
                ...
                if line:
                    if line.startswith(TRACE_TEXT_HEADER):
                        file_count += 1
                        ...
                        # Open a new file and write suffix
                        html_file = open(file_prefix + str(file_count) + '.' + file_suffix, 'wb')
                        html_file.write(html_prefix.replace('{{SYSTRACE_TRACE_VIEWER_HTML}}',
                                                            trace_viewer_html))
                        html_file.write('<!-- BEGIN TRACE -->\n')

                        # Write results other than trace
                        for result in trace_results:
                            # output_file = open(output_file_name+'.'+result.source_name, 'w+')
                            if result.source_name == 'systemTraceEvents':
                                html_file.write('  <script class="trace-data" type="application/text">\n')
                                html_file.write(_ConvertToHtmlString(result.raw_data[file_count - 1].raw_data))
                            elif result.source_name == 'systraceController':
                                continue
                            else:
                                html_file.write('  <script class="trace-data" type="application/text">\n')
